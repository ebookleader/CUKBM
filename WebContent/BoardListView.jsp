...
<html>
<head>
<link href="css/boardlistview.css" type="text/css" rel="stylesheet">
<meta charset="EUC-KR">
<title>게시판</title>
</head>
<body>
      ...
      <tr>
         ...
         <td>
         	<c:choose>
         		...
         		<c:otherwise>
					<a href='BoardItem?SEQ_NO=${Board_List.seqNo[cnt]}&SUBJECT=${param.SUBJECT}'>${Board_List.title[cnt]}</a>
				</c:otherwise>
			</c:choose>
         </td>
...
<c:if test="${!Board_List.lastPage}">
   <div class="next_page">
   	<a href='Board?LAST_SEQ_NO=${param.LAST_SEQ_NO}&SUBJECT=${param.SUBJECT}&TEAM=${param.TEAM}'>다음페이지</a>
   </div>   
</c:if>
    </section>
